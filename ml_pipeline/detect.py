...
# ── Real module imports ───────────────────────────────────────
# Comment these out if testing without OCR or backend
try:
...
smoothed_boxes   = {}   # key → smoothed coords for trash
smoothed_persons = {}   # key → smoothed coords for persons

last_drawn_persons  = []
last_drawn_vehicles = []
last_drawn_trash    = []

last_known_persons  = []   # persons from last detection frame
last_known_vehicles = []   # vehicles from last detection frame

# ── Scene analytics ───────────────────────────────────────────
# Tracks current trash on screen regardless of state machine
# This catches pre-existing trash and untracked items
current_trash_counts = {}   # {label: count} updated every frame
current_zone_counts  = {}   # {zone_name: count} updated every frame

frame_count   = 0
prev_time     = 0
frame_h       = 0   # set after first frame read
frame_w       = 0


# ── Zone config ───────────────────────────────────────────────
# Divides frame into a 3x2 grid of named zones
# Useful for dashboard hotspot reporting
ZONE_COLS = 3
ZONE_ROWS = 2
ZONE_NAMES = {
...
def get_zone(box):
    """Returns zone name string for a bounding box."""
...
def draw_zone_grid(frame):
    """
    Draws a faint 3x2 zone grid on frame for spatial reference.
    Helps judges see which zone has the most litter.
    """
    h, w = frame.shape[:2]
    col_w = w // ZONE_COLS
    row_h = h // ZONE_ROWS
...
def update_object_state(key, current_box, prev_box, persons, vehicles, frame):
    ...
    if key not in object_states:
        object_states[key] = {
            ...
            "label":          None,
            "confidence":     0.0,
            "first_seen":     frame_count,   # for dwell time tracking
        }
...
def run_trash_detection(frame, persons, vehicles):
    """
    Runs ByteTrack trash detection every frame.
    Also counts trash types and zones currently visible (scene analytics).
    Returns (events, trash_type_counts, zone_counts).
    """
    ...
    # Scene analytics — reset each frame
    trash_type_counts = {}
    zone_counts       = {}
    ...
    for box in trash_detection.boxes:
        ...
        # ── Scene analytics: count regardless of state ───────
        ...
        # Show dwell time for STATIONARY items
        dwell_str = ""
        if state == "STATIONARY":
            sep_f = object_states.get(key, {}).get("sep_frame", frame_count)
            secs  = round((frame_count - sep_f) / 30, 0)
            dwell_str = f"|{int(secs)}s"
...
def draw_hud(frame, fps, persons, vehicles, events, trash_type_counts, zone_counts):
    """All HUD drawing in one place for clarity."""
    ...
    cv2.putText(frame, f"Trash visible: {sum(trash_type_counts.values())}",
    ...
    # Scene analytics - trash type breakdown
    y += 8
    ...
    # Hotspot zone (most littered zone)
    if zone_counts:
...
while True:
    ...
    # ── SKIPPED FRAME ─────────────────────────────────────────
    # Person detection skipped to save compute
    # Trash tracking still runs every frame (keeps ByteTrack IDs stable)
    if frame_count % SKIP_FRAMES != 0:

        # Redraw last known person/vehicle boxes
    ...
    # Run trash detection + scene analytics
